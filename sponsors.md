---
layout: page
title: Sponsors
permalink: /sponsors/
main_nav: true
---

We thank all the <b> sponsors and other funding sources</b> that made SBW2026 possible:

<div class="image-grid">
    <a href="https://www.scilifelab.se/">
    <img src="{{ site.baseurl }}/assets/sponsors/SciLifeLab_Logotype_Green_POS.png" alt="SciLifeLab">
    </a>
    <a href="https://www.nordic-compbio.org/">
    <img src="{{ site.baseurl }}/assets/sponsors/NordicCompBio.png" alt="NordicCompBio">
    </a>
</div>

<br>

<h4>Sponsorship</h4>

This partnership not only underscores our events prestige but also offers our sponsors unparalleled access to a diverse and engaged community at the forefront of bioinformatics research and development.
As a sponsor, you:
<ul>
    <li>Contribute to increased collaboration among scientists from higher education institutions and industrial organizations, including startups across the Nordics.</li>
    <li>Build your brand awareness at our digital and live events.</li>
    <li>Network with a wider audience.</li>
    <li>Get a chance to find you new team members through recruitment, with the SBW event(s) as a great cross-border and multi-disciplinary platform.</li>
    <li>Enable exchange and collaboration with other organizations.</li>
</ul>

<h4>Sponsorship Levels</h4>

There are multiple ways to support and sponsor SBW2026. Depending on what level of sponsorship is most suitable for you organization, we are able to provide different opportunities in return:

<table>
<thead>
    <tr>
    <th></th>
    <th>Basic</th>
    <th>Bronze</th>
    <th>Silver</th>
    <th>Gold</th>
    </tr>
</thead>
<tbody>
    <tr>
    <td>Cost(sek)</td>
    <td>5,000</td>
    <td>10,000</td>
    <td>30,000</td>
    <td>50,000</td>
    </tr>
    <tr>
    <td><b>Benefits</b></td>
    <td></td>
    <td></td>
    <td></td>
    <td></td>
    </tr>
    <tr>
    <td>Logo on website</td>
    <td>&#x2713;</td>
    <td>&#x2713;</td>
    <td>&#x2713;</td>
    <td>&#x2713;</td>
    </tr>
    <tr>
    <td>Conference attendance</td>
    <td>1</td>
    <td>2</td>
    <td>3</td>
    <td>5</td>
    </tr>
    <tr>
    <td>Booth at the event</td>
    <td></td>
    <td>&#x2713;</td>
    <td>&#x2713;</td>
    <td>&#x2713;</td>
    </tr>
    <tr>
    <td>5 minute talk</td>
    <td></td>
    <td></td>
    <td>&#x2713;</td>
    <td></td>
    </tr>
    <tr>
    <td>10 minute talk</td>
    <td></td>
    <td></td>
    <td></td>
    <td>&#x2713;</td>
    </tr>
</tbody>
</table>
